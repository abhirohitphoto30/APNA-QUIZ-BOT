...
const TL_LABELS = { 10:'10s',15:'15s',20:'20s',25:'25s',30:'30s',40:'40s',45:'45s',50:'50s',60:'1m',90:'1.5m',120:'2m',180:'3m',300:'5m' };
...
// Last non-empty line (used as poll title / options message header)
function lastLine(q) {
  const ls = q.split('\n').map(l=>l.trim()).filter(Boolean);
  return ls[ls.length - 1] || q;
}

async function getUserSettings(uid) {
  return (await store.get('settings:'+uid)) || { negativeMarking:0, timeLimit:30, shuffle:false };
}
async function getSession(cid) { return store.get('session:'+cid); }
async function saveSession(cid, s) { return store.set('session:'+cid, s, 7200); }
async function deleteSession(cid) {
...
export async function handleCreateQuiz(ctx) {
  await ctx.reply(
    ...
    '\uD83D\uDCCC Mark correct with \u2705 | Ex: for explanation\n\uD83D\uDC46 *Now send your .txt file!*',
    { parse_mode:'Markdown' }
  );
}
...
export async function handleDocument(ctx) {
  ...
  try {
    ...
    if (!questions.length)
      return ctx.api.editMessageText(ctx.chat.id, msg.message_id, '\u274C No valid questions found. Use /createquiz to see formats.');
    if (questions.length > 300)
      return ctx.api.editMessageText(ctx.chat.id, msg.message_id, '\u274C Too many questions ('+questions.length+'). Max 300.');
...
export async function handleCallback(ctx) {
  ...
  // ans:sessionId:optionIdx
  if (data.startsWith('ans:')) {
    const p = data.split(':');
    return handleInlineAnswer(ctx, p[1], parseInt(p[2],10));
  }
  ...
  // Admin skip for group (fallback when poll_closed not received)
  if (data.startsWith('skipq:')) {
    const [,sid,qIdxStr] = data.split(':');
    const sess = await getSession(ctx.chat.id);
    if (!sess || sess.sessionId!==sid) return;
    if (sess.startedBy!==ctx.from.id)
      return ctx.answerCallbackQuery({text:'\u26A0\uFE0F Only quiz creator can skip!',show_alert:true}).catch(()=>{});
...
// ─── SEND QUESTION ────────────────────────────────────────────────────────────
//
// FORMAT — same in PRIVATE and GROUP:
//   Message 1 (text): Full question text exactly as in .txt file
//                     Q{n}/{total}: Q{n}. [question with newlines]
//                     + A) B) C) D) if options are long
//
//   Message 2 (poll or buttons): Quiz poll (group) or inline keyboard (private)
//                     [n/total] last line of question
//                     + options
//
async function sendQuestion(api, session, quiz) {
  ...
  // ── MESSAGE 1: Full question text (same for both private & group) ──────────
  let questionText = 'Q'+num+'/'+total+': Q'+num+'. '+q.question;
  if (long) {
    questionText += '\n\nOptions:';
    q.options.forEach((o,i) => { questionText += '\n  '+ALPHA[i]+') '+o; });
  }
  await api.sendMessage(chatId, questionText);

  // Small delay to ensure message 1 arrives before message 2
  await delay(300);

  // ── MESSAGE 2: Options (poll for group, inline keyboard for private) ────────
  ...
  if (isGroup) {
    // Group: Telegram native quiz poll
    try {
      ...
      // Fallback: send inline keyboard instead
      const kb = new InlineKeyboard();
      if (long) {
        q.options.forEach((_,i)=>kb.text(ALPHA[i],'ans:'+sid+':'+i));
        kb.row();
      } else {
        q.options.forEach((o,i)=>kb.text(o.slice(0,64),'ans:'+sid+':'+i).row());
      }
      kb.text('\uD83D\uDED1 End Quiz','endquiz:'+sid);
      const sentMsg = await api.sendMessage(chatId, pollTitle, {reply_markup:kb});
      session.currentMsgId = sentMsg.message_id;
    }
    // Admin skip button (fallback if poll_closed update not received by webhook)
    await delay(200);
    const skipKb = new InlineKeyboard()
      .text('\u23ED\uFE0F Next Question (Admin Only)','skipq:'+sid+':'+session.currentIndex);
    await api.sendMessage(chatId, '\u23F1\uFE0F Q'+num+'/'+total+' \u2014 '+tlLabel(settings.timeLimit), {reply_markup:skipKb}).catch(()=>{});
  } else {
    ...
    kb.text('\uD83D\uDED1 End Quiz','endquiz:'+sid);
    const optionsText = '\u2753 ['+num+'/'+total+'] '+lastLine(q.question)+'\n\u23F1\uFE0F '+tlLabel(settings.timeLimit);
    const sentMsg = await api.sendMessage(chatId, optionsText, {reply_markup:kb});
    session.currentMsgId = sentMsg.message_id;
  }
...
async function handleInlineAnswer(ctx, sessionId, optionIdx) {
  ...
  // Edit the OPTIONS message (message 2) to show result
  const num   = qIdx+1;
  const total = quiz.questions.length;
  let resultText = '\u2753 ['+num+'/'+total+'] '+lastLine(q.question)+'\n\n';
  q.options.forEach((o,i) => {
    const lbl = long ? ALPHA[i]+') '+o : o;
    if (i===optionIdx&&isCorrect)        resultText += '\u2705 '+lbl+' \u2190 Correct \u2713\n';
    else if (i===optionIdx&&!isCorrect)  resultText += '\u274C '+lbl+' \u2190 Your answer \u2717\n';
    else if (i===q.correctIndex)         resultText += '\u2611\uFE0F '+lbl+' \u2190 Correct answer\n';
    else                                 resultText += '\u25AB\uFE0F '+lbl+'\n';
  });
  if (!sess.isGroup) {
  ...
  if (sess.currentMsgId) {
    await ctx.api.editMessageText(sess.chatId, sess.currentMsgId, resultText).catch(()=>{});
  }
...
export async function handlePollAnswer(ctx) {
  ...
  const quiz = await store.get('sqz:'+sess.sessionId) || await store.get('quiz:'+sess.quizId);
  if (!quiz) return;
  const qIdx=pollMeta.questionIndex, q=quiz.questions[qIdx];
  if (!q) return;
  const userId=pa.user.id, optionIdx=pa.option_ids?.[0]??-1, isCorrect=optionIdx===q.correctIndex;
...
// ─── POLL CLOSED → send explanation + advance quiz ────────────────────────────
  ...
  if (sess.isGroup) {
    ...
    if (!entries.length) { lb+='\n_No one attempted._'; }
      ...
      entries.slice(0,10).forEach(([uid,p],i)=>{
        ...
        const fin=Math.max(0,(p.correct||0)-penalty);
...
// Sends each question + its poll TOGETHER (no batching) with delays between them.
async function startAnonymousPolls(ctx, quiz) {
  ...
  for (let i=0; i<total; i++) {
    ...
    try {
      ...
      let contextText='Q'+(i+1)+'/'+total+': Q'+(i+1)+'. '+q.question;
      if (long) {
        contextText+='\n\nOptions:';
        q.options.forEach((o,idx)=>{ contextText+='\n  '+ALPHA[idx]+') '+o; });
      }
      await ctx.api.sendMessage(ctx.chat.id, contextText);
      // Small delay — ensures question arrives before poll
      await delay(300);
      // Message 2: Quiz poll (immediately below question)
      ...
      sent++;
      // Delay before next question (prevent flood)
